...
def detect_cloudflare(ips, cname, headers):
    ...
    if ip_match:
        detection_methods.append({
            'method': 'ip_range',
            'confidence': 90,
            'evidence': f'IP in Cloudflare ranges: {ips}'
        })
        confidence_score += 90
        ...
        try:
            ...
            if cert:
                ...
                result["cert_subject"] = subject.get(
                    'commonName') if isinstance(subject, dict) else subject
                result["cert_issuer"] = issuer.get(
                    'commonName') if isinstance(issuer, dict) else issuer
            ...
            if final_status is None or (isinstance(final_status, int) and final_status >= 400):
                ...
                try:
                    ...
                    if retry_attempts > 0:
                        sock2, err2 = tcp_connect_with_retry(host, port, timeout, retry_attempts, retry_delay)
                    else:
                        sock2, err2 = tcp_connect(host, port, timeout)
                    if sock2:
                        ssl_sock2 = ctx.wrap_socket(
                            sock2, server_hostname=host)
...
def parse_args():
    ...
    p.add_argument('--parallel-workers', type=int, default=3,
...
def pretty_print(domain, ips, res80, res443):
    ...
    if not res80["open"]:
        ...
        if res80.get("http_status") is not None:
            print(
                f"    HTTP: {res80['http_status']}  {res80.get('http_status_line') or ''}")
        else:
            print(
                f"    HTTP: no valid HTTP response (error: {res80.get('http_error')})")
...
def print_table(results):
    ...
    headers = ["Domain", "CNAME", "IPs", "Port80", "Port443",
               "80->443", "HTTPS_200", "Page", "Cloudflare"]
    rows = []
    for r in results:
        ...
        if cf_detection.get('detected'):
            ...
            cf = "no"
        rows.append([r['domain'], cname, ips, port80,
                    port443, redirects, https_ok, page, cf])
...
def output_results(results, output_format='table', output_file=None):
    ...
    try:
        if output_format == 'table':
            ...
            headers = [
                'domain', 'cname', 'ips', 
                'port80_status', 'port443_status',
                'redirect_80_to_443', 'https_200', 
                'page', 'cloudflare', 'cloudflare_confidence'
            ]
...
def probe_domain(d, config=None):
    ...
    res80 = check_port(d, 80, timeout=connect_timeout, 
                       retry_attempts=config.get('retry_attempts', 0),
                       retry_delay=config.get('retry_delay', 0.5))
    res443 = check_port(d, 443, timeout=connect_timeout,
                        retry_attempts=config.get('retry_attempts', 0),
                        retry_delay=config.get('retry_delay', 0.5))
...
def main():
    ...
    print(f"Configuration: Connect timeout={config['connect_timeout']}s, "
          f"Read timeout={config['read_timeout']}s, "
          f"Retry attempts={config['retry_attempts']}, "
          f"Retry delay={config['retry_delay']}s", file=sys.stderr)
